...
namespace LeverageTechnology.Client
{
    public class JwtAuthenticationStateProvider : AuthenticationStateProvider, ILoginService
    {
        ...
        private readonly string USER_FULL_NAME_KEY = "userFullName";
        ...
        public async Task Login(LoginResultDto dto)
        {
            try
            {
                await _localStorage.SetItemAsStringAsync(TOKEN_KEY, dto.Token);
                await _localStorage.SetItemAsStringAsync(USER_FULL_NAME_KEY, dto.Name);
            }
            catch { }
